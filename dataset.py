...
class TextDataset(Dataset):
    def __init__(self, args):
        ...
        if self.args.del_bad_word:
            with open(args.bad_word_path, 'r', encoding='utf-8') as f:
        ...
        if args.model_name == 'RNN':
            ...
            print('Prepareing complete. got date file, character number: ', file_len)
            # Get all unique characters in the file
            all_characters = sorted(set(self.data))
            ...
            self.lyrics = self.data
            self.data_len = len(self.lyrics)
            print(self.lyrics[:10])
            print(self.data_len)
